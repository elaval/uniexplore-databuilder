...
var filename = "./inputData/v_output_estudiantes_x_carreras_extendido.txt";
...
var output_properties = ["id","semestre","genero","unidad","carrera","ano_ficha","prom_paa","ptje_pond_selec","dependencia","prom_notas","actividades_sakai","actividades_aleph","actividades_ezproxy"];
...
var saveCarreras = function(carreras, carrerasObj) {
	...
	var outdata = "carrera\tn\tfilename\n";

	_.each(carreras, function(d) {
		var filename = escape_filename(d);
		var n = carrerasObj[d].length;
		outdata += d+"\t"+n+"\t"+filename+".txt\n";
	})

	fs.writeFileSync("carreras.txt", outdata);
